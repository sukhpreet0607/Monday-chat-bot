...
const SAMPLE_DATA = {
  projects: [
    ...
    {
      project_id: "PRJ005",
      project_name: "Customer Survey Analysis",
      owner_id: "EMP004",
      status: "In Progress",
      ...
      if (queryLower.includes("status") && queryLower.includes("project")) {
        ...
        if (projectName) {
          const project = projectsData.find(
            (p) => p.project_name === projectName.project_name
          );
          const projectTasks =
            tasksData?.filter(
...
${projectTasks
  ...
  Assigned to: ${getEmployeeName(task.assignee_id)}`
  )
  .join("\n")}
          `);
        } else {
          setResponse(
            "I couldn't find that project. Please check the project name and try again."
          );
        }
      } else if (
          ...
          .map(
            (project) =>
              `- ${project.project_name} (${
                project.status
              }) - Owner: ${getEmployeeName(project.owner_id)}`
          )
          .join("\n");
        ...
        setResponse(
          "I understand you're asking about projects. Try asking:\n1. 'What's the status of [Project Name]?'\n2. 'Show me all projects'"
        );
      }
    } catch (err) {
      console.error("Error processing query:", err);
      setError(
        "Sorry, I encountered an error processing your query. Please make sure all required data is loaded."
      );
    }
      ...
      <div className="space-y-6">
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-gray-800">
            Monday.com Query Assistant
          </h2>
          <p className="text-gray-500">
            Sample data loaded - you can also upload your own Monday.com export
            files
          </p>
        </div>
